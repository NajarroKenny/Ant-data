...
def search(country, type, f=None, interval='month'):
    ...
    s.aggs.bucket('dates', 'date_histogram', field=type, interval=interval) \
        .bucket('communities', 'terms', field='community.community_id')
...
def df(country, f=None, interval='month'):
    ...
    df = DataFrame(df.values, index=idx, columns=['opened', 'closed']).swaplevel('date','community_id').sort_index()
    df['open'] = df['opened'].groupby(['community_id']).cumsum()- df['closed'].groupby(['community_id']).cumsum()
