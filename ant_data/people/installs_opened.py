...
def search(country, start=None, end=None, f=None, interval='month', model=None, version=None, model_version=None, system_type=None, open_type=None, by='model'):
  ...
  if start is not None and end is not None:
    filter.append(Q('range', installations__opened={ 'gte': start, 'lt': end }))
  elif start is not None:
    filter.append(Q('range', installations__opened={ 'gte': start }))
  elif end is not None:
    filter.append(Q('range', installations__opened={ 'lt': end }))
  else:
  ...
  if open_type is not None and open_type != []:
    open_type = open_type if isinstance(open_type, list) else [ open_type ]
    filter.append(Q('terms', installations__open_type=open_type))
  ...
  s.aggs.bucket('installations', 'nested', path='installations') \
    ...
    .bucket('dates', 'date_histogram', field='installations.opened', interval=interval, min_doc_count=1) \
    .bucket('by', 'terms', field=f'installations.{by}')

  return s[:0].execute()


def df(country, start=None, end=None, f=None, interval='month', model=None, version=None, model_version=None, system_type=None, open_type=None, by='model'):
  response = search(country, start=start, end=end, f=f, interval=interval, model=model, version=version, model_version=model_version, system_type=system_type, open_type=open_type, by=by)
